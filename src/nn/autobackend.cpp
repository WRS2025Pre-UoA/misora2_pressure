...
#include "misora2_pressure/utils/augment.hpp"
#include "misora2_pressure/constants.hpp"
#include "misora2_pressure/utils/common.hpp"
#include "misora2_pressure/utils/ops.hpp"
...
AutoBackendOnnx::AutoBackendOnnx(const char* modelPath, const char* logid, const char* provider)
    ...
    // post init imgsz
    auto imgsz_iterator = base_metadata.find(Constants::MetadataConstants::IMGSZ);
    if (imgsz_iterator != base_metadata.end()) {
        // parse it and convert to int iterable
        std::vector<int> imgsz = Common::convertStringVectorToInts(Common::parseVectorString(imgsz_iterator->second));
        // set it here:
        if (imgsz_.empty()) {
            imgsz_ = imgsz;
        }
    ...
    // post init stride
    auto stride_item = base_metadata.find(Constants::MetadataConstants::STRIDE);
    if (stride_item != base_metadata.end()) {
        // parse it and convert to int iterable
        int stide_int = std::stoi(stride_item->second);
        // set it here:
        if (stride_ == Constants::OnnxInitializers::UNINITIALIZED_STRIDE) {
            stride_ = stide_int;
        }
    }
    else {
    ...
    // post init names
    auto names_item = base_metadata.find(Constants::MetadataConstants::NAMES);
    if (names_item != base_metadata.end()) {
        // parse it and convert to int iterable
        std::unordered_map<int, std::string> names = Common::parseNames(names_item->second);
    ...
    if (nc_ == Constants::OnnxInitializers::UNINITIALIZED_NC && !names_.empty()) {
        nc_ = names_.size();
    }
    else {
    ...
    // task init:
    auto task_item = base_metadata.find(Constants::MetadataConstants::TASK);
...
std::vector<YoloResults> AutoBackendOnnx::predict_once(cv::Mat& image, const float& conf, const float& iou,  const float& mask_threshold, int conversionCode, bool verbose) {
    ...
    Augment::letterbox(image, preprocessed_img, new_shape, cv::Scalar(), auto_, scaleFill, true, getStride());
    fill_blob(preprocessed_img, blob, inputTensorShape);
    int64_t inputTensorSize = Common::vector_product(inputTensorShape);
    ...
    if (task_ == Constants::YoloTasks::SEGMENT) {
    ...
    else if (task_ == Constants::YoloTasks::DETECT) {
        ImageInfo img_info = { image.size() };
        std::vector<int64_t> outputTensor0Shape = outputTensors[0].GetTensorTypeAndShapeInfo().GetShape();
        float* all_data0 = outputTensors[0].GetTensorMutableData<float>();
        cv::Mat output0 = cv::Mat(cv::Size((int)outputTensor0Shape[2], (int)outputTensor0Shape[1]), CV_32F, all_data0).t();  // [bs, features, preds_num]=>[bs, preds_num, features]
    ...
    else if (task_ == Constants::YoloTasks::POSE) {
    ...
    int& iw, int& ih, int& mw, int& mh, int& masks_features_num, float mask_threshold /* = 0.5f */)
{
    ...
    for (int r = 0; r < rows; ++r)
    {
        ...
        if (max_conf > conf_threshold)
        {
            ...
            cv::Rect_<float> scaled_bbox = Ops::scale_boxes(getCvSize(), bbox, image_info.raw_size);
                                          ...
                                          int& class_names_num, const float& conf_threshold, const float& iou_threshold)
{
    ...
    std::tie(boxes, confidences, class_ids, rest) = Ops::non_max_suppression(output0, class_names_num, output0.cols, conf_threshold, iou_threshold);
    ...
    for (size_t i = 0; i < boxes.size(); i++) {
        ...
        auto scaled_bbox = Ops::scale_boxes(img1_shape, bbox, image_info.raw_size);
...
//        scale_coords(img1_shape, kpt, image_info.raw_size);
        ...
        std::vector<float> kpt = Ops::scale_coords(img1_shape, rest[i], image_info.raw_size);
...

{
    ...
    cv::Rect_<float> downsampled_bbox = Ops::scale_boxes(img0_shape, bound_float, downsampled_size);
    cv::Size bound_size = cv::Size(mw, mh);
    Ops::clip_boxes(downsampled_bbox, bound_size);
    ...
    cv::Mat resized_mask;
    cv::Rect_<float> input_bbox = Ops::scale_boxes(img0_shape, bound_float, img1_shape);
    cv::resize(sigmoid_mask, resized_mask, img1_shape, 0, 0, cv::INTER_LANCZOS4);
    cv::Mat pre_out_mask = resized_mask(input_bbox);
    cv::Mat scaled_mask;
    Augment::scale_image2(scaled_mask, resized_mask, img0_shape);
