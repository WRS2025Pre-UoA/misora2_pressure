    ...
    : modelPath_(modelPath)
{
    ...
    if (provider == Constants::OnnxProviders::CUDA.c_str()) {  // strcmp(provider, Constants::OnnxProviders::CUDA.c_str()) == true strcmp(provider, "cuda") // (providerStr == "cuda")
    ...
    else if (provider == Constants::OnnxProviders::CPU) {  // strcmp(provider, Constants::OnnxProviders::CPU.c_str()) == true) (providerStr == "cpu") {
        // "cpu" by default
    }
    else
    {
    ...
    #ifdef _WIN32
        auto modelPathW = Common::get_win_path(modelPath);  // For Windows (wstring)
