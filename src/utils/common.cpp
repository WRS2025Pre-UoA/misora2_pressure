...
// С++ 14 version
//#define _SILENCE_CXX17_CODECVT_HEADER_DEPRECATION_WARNING
std::wstring Common::get_win_path(const std::string& modelPath) {
#ifdef _WIN32
...
std::vector<std::string> Common::parseVectorString(const std::string& input) {
...
std::vector<int> Common::convertStringVectorToInts(const std::vector<std::string>& input) {
...
std::unordered_map<int, std::string> Common::parseNames(const std::string& input) {
...
int64_t Common::vector_product(const std::vector<int64_t>& vec) {
    int64_t result = 1;
