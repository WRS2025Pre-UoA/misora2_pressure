...
const std::pair<float, cv::Point2f> Augment::DEFAULT_RATIO_PAD = std::make_pair(-1.0f, cv::Point2f(-1.0f, -1.0f));

void Augment::letterbox(const cv::Mat& image,
    cv::Mat& outImage,
...
cv::Mat Augment::scale_image(const cv::Mat& resized_mask, const cv::Size& im0_shape, const std::pair<float, cv::Point2f>& ratio_pad) {
...
void Augment::scale_image2(cv::Mat& scaled_mask, const cv::Mat& resized_mask, const cv::Size& im0_shape, const std::pair<float, cv::Point2f>& ratio_pad) {
    cv::Size im1_shape = resized_mask.size();
