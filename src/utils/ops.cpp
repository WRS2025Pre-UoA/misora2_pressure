...
void Ops::clip_boxes(cv::Rect& box, const cv::Size& shape) {
...
void Ops::clip_boxes(cv::Rect_<float>& box, const cv::Size& shape) {
...
void Ops::clip_boxes(std::vector<cv::Rect>& boxes, const cv::Size& shape) {
    for (cv::Rect& box : boxes) {
        clip_boxes(box, shape);
    }
}

void Ops::clip_boxes(std::vector<cv::Rect_<float>>& boxes, const cv::Size& shape) {
...
cv::Rect_<float> Ops::scale_boxes(const cv::Size& img1_shape, cv::Rect_<float>& box, const cv::Size& img0_shape,
...
void Ops::clip_coords(std::vector<float>& coords, const cv::Size& shape) {
...
std::vector<float> Ops::scale_coords(const cv::Size& img1_shape, std::vector<float>& coords, const cv::Size& img0_shape)
{
...
cv::Mat Ops::crop_mask(const cv::Mat& mask, const cv::Rect& box) {
    int h = mask.rows;
    int w = mask.cols;
...
Ops::non_max_suppression(const cv::Mat& output0, int class_names_num, int data_width, double conf_threshold,
                    float iou_threshold) {
